 ...
 * 
 * Code is mostly copied from princeton. Code is changed so that for one edge read, the code creates two weighted directed edges. This is because data in is considered to be double directed / symmetric. 
 * 
 * 
 * @author Tom K. Axberg
 * @version 1.0
 ...
 * returning the indegree or outdegree of a vertex, the number of vertices
 ...
 * proportional to the outdegree of the vertex.) Constructing an empty
 * edge-weighted digraph with <em>V</em> vertices takes &Theta;(<em>V</em>)
 * time; constructing an edge-weighted digraph with <em>E</em> edges and
 ...
 * 
 */
public class DoubleEdgeWeightedDigraph {
    ...
    public DoubleEdgeWeightedDigraph(In in) {
        if (in == null)
        ...
        try {
            ...
            indegree = new int[V];
            adj = (Bag<DirectedEdge>[]) new Bag[V * 2];
            for (int v = 0; v < V * 2; v++) {
            ...
            for (int i = 0; i < E; i++) {
                ...
                addEdge(new DirectedEdge(v, w, weight)); // Directed Weighted Edge is created
                addEdge(new DirectedEdge(w, v, weight)); // Directed Weighted Edge is created to make connections
                                                         // symmetric.
            }
